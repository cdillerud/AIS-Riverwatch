...
if [[ ! -f backend/.env.pi ]]; then
    ...
    cat > backend/.env.pi <<EOF
# River Watch - Pi-local backend env (auto-generated by pi-local-install.sh)
# Re-run the installer with backend/.env.pi deleted to regenerate.
