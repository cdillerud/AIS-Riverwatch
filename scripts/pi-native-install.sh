...
server {
    ...
    # WebSocket upgrade for AIS data (App.js connects here in production builds)
    # Backend exposes both /ws/ais and /api/ws/ais for compatibility.
    location /ws/ {
    ...
    # NOTE: must come before /api/ block so nginx prefers this longer prefix.
    location /api/ws/ {
    ...
    # Catch bare /api/ws (no trailing slash) and answer with 308 so the
    # browser preserves the WS upgrade method instead of nginx returning 301.
